export const purchaseTransactions = async () => {
  // Simulates fetching customer purchase data asynchronously
  const data = new Promise((resolve) => {
    setTimeout(() => {
      // Resolving with an array of mock customer transactions after 1 second
      resolve([
        
          // Each object represents a customer's purchase transaction
          ...
          { "id": 31, "customerId": 12, "customerName": "Arjun", "amount": 2200, "date": "2025-02-30" } // Large transaction example
        ]
      );
    }, 1000); // Simulating API response delay of 1 second
  });

  return data; // Returning the resolved data (mock transactions)
};
